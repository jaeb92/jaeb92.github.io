...
MapReduce 라는 건 대용량의 데이터를 처리하기 위해 2004년에 구글에서 발표한 소프트웨어 프레임워크 모델입니다. 대용량의 데이터를 클러스터링 환경으로 분산시켜서 병렬적으로 처리될 수 있도록 하는 것입니다. 그렇기 때문에 하나의 컴퓨터에서 대량의 데이터를 처리하는 것보다 속도는 훨씬 증가하겠죠? 물론 그에 따라 발생하는 부작용들은 당연히 존재합니다.
...
##### ~~<center>🤣 여기서 map은 그 🗺(map)  아닙니다 ㅎㅎ </center>~~
